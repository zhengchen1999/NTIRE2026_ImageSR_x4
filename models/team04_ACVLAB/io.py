...
from models.team4_HAT.model import HAT
...
def forward(img_lq, model, tile=None, tile_overlap=32, scale=4):
    ...
    window_size = 16  # HAT model's window size
    
    if tile is None:
        # Handle whole image processing with padding
        b, c, h, w = img_lq.size()
        pad_h = (window_size - h % window_size) % window_size
        pad_w = (window_size - w % window_size) % window_size
        
        if pad_h > 0 or pad_w > 0:
            img_lq_padded = F.pad(img_lq, (0, pad_w, 0, pad_h))
            output = model(img_lq_padded)
            # Crop back to the original size (accounting for scale factor)
        ...
        # For tile-based processing, ensure each tile is divisible by window_size
        b, c, h, w = img_lq.size()
        tile = min(tile, h, w)
        # Make sure tile size is divisible by window_size
        tile = (tile // window_size) * window_size
        tile_overlap = min(tile_overlap, tile)
        sf = scale
        ...
        for h_idx in h_idx_list:
            for w_idx in w_idx_list:
                ...
                p_h, p_w = in_patch.size(2), in_patch.size(3)
                pad_h = (window_size - p_h % window_size) % window_size
                pad_w = (window_size - p_w % window_size) % window_size
                
                if pad_h > 0 or pad_w > 0:
                    in_patch_padded = F.pad(in_patch, (0, pad_w, 0, pad_h))
                    out_patch = model(in_patch_padded)
                    # Crop padding
                    out_patch = out_patch[:, :, :p_h*scale, :p_w*scale]
                else:
                    out_patch = model(in_patch)
