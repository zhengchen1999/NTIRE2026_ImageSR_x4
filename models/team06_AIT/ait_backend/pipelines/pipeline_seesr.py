...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def __call__(
        ...
        degradation_token_adapter=None,
        degradation_features=None,
        use_dynamic_degradation_token=False,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                step_ram_encoder_hidden_states = ram_encoder_hidden_states
                if (
                    use_dynamic_degradation_token
                    and degradation_token_adapter is not None
                    and degradation_features is not None
                ):
                    base_tokens = ram_encoder_hidden_states[:, :-1, :]
                    adapter_dtype = next(degradation_token_adapter.parameters()).dtype
                    step_features = degradation_features.to(device=latents.device, dtype=adapter_dtype)
                    step_t = torch.full(
                        (step_features.shape[0],),
                        int(t.item() if isinstance(t, torch.Tensor) else t),
                        device=latents.device,
                        dtype=torch.long,
                    )
                    step_deg_token = degradation_token_adapter(step_features, timesteps=step_t).unsqueeze(1)
                    step_deg_token = step_deg_token.to(dtype=base_tokens.dtype)
                    if step_deg_token.shape[0] != base_tokens.shape[0]:
                        if base_tokens.shape[0] % step_deg_token.shape[0] != 0:
                            raise ValueError(
                                f"Dynamic degradation token batch ({step_deg_token.shape[0]}) "
                                f"cannot align with RAM token batch ({base_tokens.shape[0]})."
                            )
                        repeat_factor = base_tokens.shape[0] // step_deg_token.shape[0]
                        step_deg_token = step_deg_token.repeat_interleave(repeat_factor, dim=0)
                    step_tokens = torch.cat([base_tokens, step_deg_token], dim=1)
                    step_ram_encoder_hidden_states = step_tokens
                ...
                if h*w<=tile_size*tile_size: # tiled latent input
                    ...
                    down_block_res_samples, mid_block_res_sample = self.controlnet(
                        ...
                        guess_mode=guess_mode,
                        return_dict=False,
                        image_encoder_hidden_states = step_ram_encoder_hidden_states,
                    )
