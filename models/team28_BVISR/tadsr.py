...
def timestep_embedding(timesteps, dim, max_period=10000):
    half = dim // 2
    freqs = torch.exp(
        -math.log(max_period) * torch.arange(start=0, end=half, dtype=torch.float32, device=timesteps.device) / max(half, 1)
    )
...
class ResidualNoiseRefiner(nn.Module):
    def __init__(self, latent_channels=4, hidden_dim=64, time_embed_dim=64, out_scale=0.05):
        super().__init__()
        self.time_embed_dim = time_embed_dim
        self.out_scale = out_scale
        self.in_proj = nn.Conv2d(latent_channels * 2, hidden_dim, kernel_size=3, padding=1)
        ...
        self.out_proj = nn.Conv2d(hidden_dim, latent_channels, kernel_size=3, padding=1)
        nn.init.zeros_(self.out_proj.weight)
        nn.init.zeros_(self.out_proj.bias)

    def forward(self, latent, noise_pred, timesteps):
        time_embed = timestep_embedding(timesteps, self.time_embed_dim).to(latent.dtype)
        time_embed = self.time_mlp(time_embed).to(latent.dtype)[:, :, None, None]

        h = self.in_proj(torch.cat([latent, noise_pred], dim=1))
        h = h + time_embed
        h = h + self.block1(h)
        h = h + self.block2(h)
        delta = self.out_proj(F.silu(h))
        return self.out_scale * delta
...
class TADSR_gen(torch.nn.Module):
    def __init__(self, args):
        ...
        self.noise_refiner = ResidualNoiseRefiner(latent_channels=self.unet.config.out_channels)
    ...
    def load_pretrained_model(self):
        ...
        if hasattr(self.args, 'pretrained_lora_path') and self.args.pretrained_lora_path is not None:
            ...
            if "state_dict_noise_refiner" in pretrained_weights:
                print("Loading residual noise refiner weights...")
                self.noise_refiner.load_state_dict(pretrained_weights["state_dict_noise_refiner"], strict=True)
    ...
    def set_train(self):
        ...
        for n, _p in self.vae.named_parameters():
            if "lora" in n or "time" in n:
                _p.requires_grad = True
        for _p in self.noise_refiner.parameters():
            _p.requires_grad = True
    ...
    def forward(self, c_t, batch=None, timesteps=None):
        ...
        base_model_pred = self.unet(encoded_control, timesteps, encoder_hidden_states=prompt_embeds.to(torch.float32),).sample
        delta_model_pred = self.noise_refiner(encoded_control, base_model_pred, timesteps)
        model_pred = base_model_pred + delta_model_pred
        x_denoised = self.get_x0_from_res(encoded_control, model_pred, timesteps)
        output_image = (self.vae.decode(x_denoised / self.vae.config.scaling_factor).sample).clamp(-1, 1)

        aux_outputs = {
            "base_model_pred": base_model_pred,
            "delta_model_pred": delta_model_pred,
            "refined_model_pred": model_pred,
        }
        return output_image, x_denoised, prompt_embeds, neg_prompt_embeds, aux_outputs
    ...
    def save_model(self, outf):
        ...
        sd["state_dict_noise_refiner"] = self.noise_refiner.state_dict()
        torch.save(sd, outf)
...
class TADSR_test(torch.nn.Module):
    def __init__(self, args):
        ...
        self.noise_refiner.to("cuda", dtype=self.weight_dtype)
    ...
    def load_ckpt(self, model):
        ...
        if "state_dict_noise_refiner" in model:
            self.noise_refiner.load_state_dict(model["state_dict_noise_refiner"], strict=True)
    ...
    def forward(self, lq, prompt, timesteps):
        ...
        lq_latent = self.vae.encode(lq.to(torch.float32), timesteps).latent_dist.sample() * self.vae.config.scaling_factor
        ...
        if h * w <= tile_size * tile_size:
            ...
            tile_weights = self._gaussian_weights(tile_size, tile_size, 1).to(torch.float32)
            ...
            for row in range(grid_rows):
                for col in range(grid_cols):
                    ...
                    if len(input_list) == 1 or col == grid_cols-1:
                        ...
                        model_out = self.unet(input_list_t, timesteps, encoder_hidden_states=prompt_embeds.to(self.weight_dtype),).sample
                        input_list = []
                    noise_preds.append(model_out)

            noise_pred = torch.zeros(lq_latent.shape, device=lq_latent.device, dtype=torch.float32)
            contributors = torch.zeros(lq_latent.shape, device=lq_latent.device, dtype=torch.float32)
            ...
            for row in range(grid_rows):
                for col in range(grid_cols):
                    ...
                    noise_pred[:, :, input_start_y:input_end_y, input_start_x:input_end_x] += noise_preds[row*grid_cols + col].float() * tile_weights
                    contributors[:, :, input_start_y:input_end_y, input_start_x:input_end_x] += tile_weights
                    
            # Average overlapping areas with more than 1 contributor
            noise_pred /= contributors.clamp(min=1e-8)
            model_pred = noise_pred.to(lq_latent.dtype)

        delta_model_pred = self.noise_refiner(lq_latent, model_pred, timesteps)
        model_pred = model_pred + delta_model_pred
        x_denoised = self.get_x0_from_res(lq_latent, model_pred, timesteps)
        
        output_image = (self.vae.decode(x_denoised.to(torch.float32) / self.vae.config.scaling_factor).sample).clamp(-1, 1)
