...
#       will break any backward() or torch.autograd.grad-cfg100() that passes VAE.
...
def resblock2task(queue, block, time_vae=False):
    """
    ...
    if time_vae:
        # queue.append(('time_silu', inplace_nonlinearity))
        queue.append(('time_emb_proj', block.time_emb_proj))
        queue.append(('scale_shift', None))    
    queue.append(('silu', inplace_nonlinearity))
...
def build_sampling(task_queue, net, is_decoder, time_vae=False):
    """
    ...
    for i_level in resolution_iter:
        for i_block in range(block_ids):
            resblock2task(task_queue, module[i_level].resnets[i_block], time_vae)
    ...
    if not is_decoder:
        resblock2task(task_queue, net.mid_block.resnets[0], time_vae)
        attn2task(task_queue, net.mid_block.attentions[0])
        resblock2task(task_queue, net.mid_block.resnets[1], time_vae)


def build_task_queue(net, is_decoder, time_vae=False):
    """
    ...
    build_sampling(task_queue, net, is_decoder, time_vae)
...
class VAEHook:
    def __init__(self, net, tile_size, is_decoder, fast_decoder, fast_encoder, color_fix, to_gpu=False, time_vae=False):
    ...
    def __call__(self, x, timestep=None):
        ...
        try:
            ...
            if max(H, W) <= self.pad * 2 + self.tile_size:
                ...
                if self.time_vae:
                    return self.vae_tile_forward(x, timestep)
                else:
    ...
    # @perfcount
    @torch.no_grad()
    def vae_tile_forward(self, z, timestep=None):
        """
        ...
        if timestep is not None:
            if not torch.is_tensor(timestep):
                timestep = torch.tensor([timestep], dtype=torch.long, device=z.device)
            elif torch.is_tensor(timestep) and len(timestep.shape) == 0:
                timestep = timestep[None].to(z.device)
            timestep = timestep * torch.ones(z.shape[0], dtype=timestep.dtype, device=timestep.device)
        ...
        # Build task queues
        single_task_queue = build_task_queue(net, is_decoder, self.time_vae)
        ...
        interrupted = False
        if self.time_vae:
            time_emb = net.time_proj(timestep)
            time_emb = time_emb.to(dtype=list(self.net.time_embedding.parameters())[0].dtype)
            time_emb = net.time_embedding(time_emb)
            time_emb = inplace_nonlinearity(time_emb)
        else:
            time_emb = None
            ...
            for i in range(num_tiles) if forward else reversed(range(num_tiles)):
                ...
                while len(task_queue) > 0:
                    ...
                    if task[0] == 'pre_norm':
                        ...
                        tile += task[1].to(device)
                        task[1] = None
                    elif task[0] == 'time_emb_proj':
                        time_emb_res = task[1](time_emb)[:,:,None,None]
                        task_id = 0
                        while task_queue[task_id][0] != 'scale_shift':
                            task_id += 1
                        task_queue[task_id][1] = time_emb_res
                    elif task[0] == 'scale_shift':
                        scale, shift = torch.chunk(task[1], 2, dim=1)
                        task[1] = None
                        tile = tile * (scale + 1) + shift
                    else:
                        tile = task[1](tile)
                    # print('tile_type', tile.dtype)
                    pbar.update(1)
