...
class ConvNormAct(nn.Module):
    def __init__(self, dim):
        ...
        # self.layer_norm = nn.LayerNorm([dim, img_size, img_size])
...
class RHAG(nn.Module):
	...
	def forward(self, x, x_size, params):
		# return self.patch_embed(self.lrelu(self.conv(self.patch_unembed(self.residual_group(x, x_size, params), x_size)))) + x
		# return self.patch_embed(self.relu(self.conv(self.patch_unembed(self.residual_group(x, x_size, params), x_size)))) + x
...
class E4D_HATSR(nn.Module):
	...
	def __init__(self,
		...
		elif resi_connection == '1conv+norm':
			self.conv_after_body = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1)
	...
	def forward(self, x):
		...
		if self.upsampler == 'pixelshuffle':
			# for classical SR
			img = self.conv_first(img)
			img = self.conv_after_body(self.forward_features(img)) + img
			img = self.conv_before_upsample(img)
			img = self.conv_last(self.upsample(img))

		img = img / self.img_range + self.mean

		# post process
		_, _, h, w = img.size()
		img = img[:, :, 0:h - mod_pad_h * self.upscale, 0:w - mod_pad_w * self.upscale]
