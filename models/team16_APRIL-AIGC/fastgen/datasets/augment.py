...
class AugmentPipe:
    ...
    def process(self, images):
        ...
        if G_inv is not I_3:
            ...
            conv_pad = (len(Hz) + 1) // 2
            images = torch.stack([images, torch.zeros_like(images)], dim=4).reshape(N, C, images.shape[2], -1)[
                :, :, :, :-1
            ]
            images = torch.nn.functional.conv2d(
                images, conv_weight.unsqueeze(2), groups=images.shape[1], padding=[0, conv_pad]
            )
            images = torch.stack([images, torch.zeros_like(images)], dim=3).reshape(N, C, -1, images.shape[3])[
                :, :, :-1, :
            ]
            images = torch.nn.functional.conv2d(
        ...
        if self.saturation > 0:
            ...
            M = (luma_axis.ger(luma_axis) + (I_4 - luma_axis.ger(luma_axis)) * (2 ** (w.mul(self.saturation_std)))) @ M
            labels += [w]
    ...
    def __call__(self, data):
        images = data["real"]
        images, aug_cond = self.process(images)
        data["real"] = images
        aug_cond = {"aug_condition": aug_cond}
        if "condition" in data:
            data["condition"] = {"orig_condition": data["condition"], **aug_cond}
        else:
            data["condition"] = aug_cond
        return data
