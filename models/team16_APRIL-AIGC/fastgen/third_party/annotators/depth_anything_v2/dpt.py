...
class DepthAnythingV2(nn.Module):
    ...
    @torch.no_grad()
    def infer_tensor(self, image_tensor, input_size=518):
        """Process a tensor directly without numpy conversion.
        ...
        # Prepare image tensor - resize to multiple of 14
        # Calculate scale to ensure minimum size
        scale = max(input_size / h, input_size / w)

        # Match the original constrain_to_multiple_of logic
        def constrain_to_multiple_of(x, multiple=14, min_val=None):
            y = int(np.round(x / multiple) * multiple)
            if min_val is not None and y < min_val:
                y = int(np.ceil(x / multiple) * multiple)
            return y

        new_h = constrain_to_multiple_of(scale * h, 14, input_size)
        new_w = constrain_to_multiple_of(scale * w, 14, input_size)

        # Add batch dimension and resize
        image = image_tensor.unsqueeze(0)  # [1, C, H, W]
        image = F.interpolate(image, size=(new_h, new_w), mode="bicubic", align_corners=True)
        ...
        # Forward pass
        depth = self.forward(image)  # [1, H', W']

        # Interpolate back to original size
        depth = F.interpolate(depth.unsqueeze(1), size=(h, w), mode="bilinear", align_corners=True)  # [1, 1, H, W]
