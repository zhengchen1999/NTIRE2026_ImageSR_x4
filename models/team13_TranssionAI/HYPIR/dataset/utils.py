...
def load_file_meta(file_meta: Dict[str, str]) -> List[Dict[str, str]]:
    ...
    image_path_prefix = file_meta["image_path_prefix"]
    prompt_key = file_meta["prompt_key"]
    ...
    if ext == ".parquet":
        ...
        for row in df.iter_rows(named=True):
            files.append(
                {
                    "image_path": os.path.join(image_path_prefix, row[image_path_key]),
                    "prompt": row[prompt_key] if prompt_key != "none" else "",
                }
            )
    elif ext == ".txt" or ext == ".list":
        with open(file_list_path, "r") as fp:
            for line in fp:
                ...
                if line:
                    files.append(
                        {
                            "image_path": os.path.join(image_path_prefix, line),
                            "prompt": "",
                        }
                    )
    else:
