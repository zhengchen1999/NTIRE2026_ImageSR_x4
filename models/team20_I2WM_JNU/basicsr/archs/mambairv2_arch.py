...
from models.team20_I2WM_JNU.basicsr.archs.arch_util import to_2tuple, trunc_normal_
from mamba_ssm.ops.selective_scan_interface import selective_scan_fn, selective_scan_ref
from models.team20_I2WM_JNU.basicsr.utils.registry import ARCH_REGISTRY
...
class MambaIRv2(nn.Module):
    ...
    def forward(self, lq):
        ...
        with torch.no_grad():
            outputs = []
            for chop in img_chops:
                # out = self.forward_once(chop)  # image processing of each partition
                # out = forward_x8(chop, self.net_g.forward)  # image processing of each partition
                out = forward_x8(chop, self.forward_once)  # image processing of each partition
                outputs.append(out)
            _img = torch.zeros(1, C, H * scale, W * scale)
            # merge
            for i in range(ral):
