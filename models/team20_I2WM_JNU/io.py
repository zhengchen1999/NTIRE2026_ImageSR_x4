...
from models.team20_I2WM_JNU.basicsr.archs.mambairv2_arch import MambaIRv2
...
def model_ensemble(save_dir):
    ...
    dir1 = os.path.join(save_dir, '01_MambaIRv2')
    dir2 = os.path.join(save_dir, '02_SRHCnet')
...
def main(model_dir, input_path, output_path, device=None):
    ...
    output_path1 = os.path.join(output_path, '01_MambaIRv2')
    ...
    run(model, input_path, output_path1, tile, device)

    output_path2 = os.path.join(output_path, '02_SRHCnet')

    # SRHCnet_x4.pth
    print("Using SRHCnet for Inference")

    model = SRHCnetLocal()
    # model.load_state_dict(torch.load(model_dir), strict=True)

    loadnet = torch.load(model_dir.replace('MambaIRv2_x4.pth', 'SRHCnet_x4.pth'))
    if 'params_ema' in loadnet:
        keyname = 'params_ema'
    else:
        keyname = 'params'
    ...
    run(model, input_path, output_path2, tile, device)
